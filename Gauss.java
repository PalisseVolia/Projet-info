...
public class Gauss {
	...
	public static ArrayList<ArrayList<Double>> remplitmatrice(LNoeud list, LBarre lBarre) {
        ...
        for(int i=0; i<length(list.getlisteNoeuds()); i++){
				...
				for(int j=0; j<n.getTabbar().size(); j++){
					for(int a = 0; a<col.size(); a++){
						if (n.getTabbar().get(j) == col.get(a)){
							...
							for(int k = 0;  k<lBarre.getlistebarre().size();k++{
								if(lBarre.getlisteBarre().get(k).getIdentificationB()==n.getTabbar(j)){
									if(n.getIdentificationN()==lBarre.getlisteBarre().get(k).getDebut_noeud().getIdentificationN(){
										double x2 = lBarre.getlisteBarre().get(k).getFin_noeud().getabscisse();
										double y2 = lBarre.getlisteBarre().get(k).getFin_noeud().getordonnee();
									}
									else{
										double x2 = lBarre.getlistBbarre().get(k).getDebut_noeud().getabscisse();
										double y2 = lBarre.getlisteBarre().get(k).getDebut_noeud().getordonnee();
									}
								}
							}
						...
						if (("R" + n.getIdentificationN()) == col.get(a)){
							Triangle t = lTriangle.getlisteTriangle(n.gettriangleappui());
							if (n.gettrianglecote() == 1)){
								double x1 = t.getAbscisseT1();
								double y1 = t.getOrdonneeT1();
								double x2 = t.getAbscisseT2();
								double y2 = t.getOrdonneeT2();
								Ligne1.set(a, Math.cos(trouve_beta(x1, x2, y1, y2)));
								Ligne2.set(a, Math.sin(trouve_beta(x1, x2, y1, y2)));
							}
							if (n.gettrianglecote() == 2)){
								double x1 = t.getAbscisseT2();
								double y1 = t.getOrdonneeT2();
								double x2 = t.getAbscisseT3();
								double y2 = t.getOrdonneeT3();
								Ligne1.set(a, Math.cos(trouve_beta(x1, x2, y1, y2)));
								Ligne2.set(a, Math.sin(trouve_beta(x1, x2, y1, y2)));
							}
							if (n.gettrianglecote() == 3)){
								double x1 = t.getAbscisseT3();
								double y1 = t.getOrdonneeT3();
								double x2 = t.getAbscisseT1();
								double y2 = t.getOrdonneeT1();
								Ligne1.set(a, Math.cos(trouve_beta(x1, x2, y1, y2)));
								Ligne2.set(a, Math.sin(trouve_beta(x1, x2, y1, y2)));	
							}
						}
	...
	public static double[] trouve_alpha(double x1,double x2, double x3, double y1, double y2, double y3) {
	...
	public static void gaussfin(){
		...
		ArrayList<ArrayList<Double>> A = remplitmatrice(lBarre.lnoeud, lBarre);
